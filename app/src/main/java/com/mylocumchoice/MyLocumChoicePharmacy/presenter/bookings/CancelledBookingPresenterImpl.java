...
public class CancelledBookingPresenterImpl implements CancelledBookingsPresenter {

    CompletedBookingView completedBookingView;
    Activity mActivity;
    public CancelledBookingPresenterImpl(CompletedBookingView completedBookingView,Activity mActivity) {
        this.completedBookingView=completedBookingView;
        this.mActivity=mActivity;
    }

    @Override
    public void onGetBookings(JsonObject jsonObject,int pageNum) {
        if(pageNum==1) {
            completedBookingView.showProgress();
        }
        BaseApplication.getRestClient().getService().getCancelledBookings(SharedPrefManager.getInstance(mActivity).getEmail()
