...
public class DeactivatePresenter implements DeactivateVP.Presenter {

    private DeactivateVP.View mView;
    private Activity activity;

    public DeactivatePresenter(DeactivateVP.View view, Activity activity) {
        this.mView = view;
        this.activity = activity;
    }

    @Override
    public void deactivateAccount(Context context, JsonObject jsonObject) {
        mView.showProgress();
        BaseApplication.getRestClient().getService().deactivateAccount(
                SharedPrefManager.getInstance(context).getEmail()
                ,SharedPrefManager.getInstance(context).getAuthToken()
                ,jsonObject).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                try{
                    mView.hideProgress();
                    mView.onDeactivate(response);
                }catch (Exception e){
                    e.printStackTrace();
                }
