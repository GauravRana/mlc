    ...
    public void onClickDate(int col, int row) {
        ...
        if (weeks[row] != null) {
            if (attr.getCalendarType() == CalendarAttr.CalendarType.MONTH) {
                ...
                currentCal.set(java.util.Calendar.MONTH, selectedDate.month - 1);
                currentCal.set(java.util.Calendar.DAY_OF_MONTH, selectedDate.day);
                currentCal.set(java.util.Calendar.YEAR, selectedDate.year);
